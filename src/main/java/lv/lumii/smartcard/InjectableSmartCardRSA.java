...
import org.bouncycastle.tls.injection.sigalgs.MyTls13Verifier;
...
public class InjectableSmartCardRSA
        implements SigAlgAPI
{
    ...
    public byte[] sign(
            JcaTlsCrypto crypto,
            byte[] message,
            byte[] privateKey) throws IOException, Exception
    {
        try
        {
    ...
    {
        boolean inTLS = MyTls13Verifier.isPresentInCallStack();
        ...
        if (inTLS)
        {
            // TLS handshake signature (for verifying server certificate sent during Server Hello) is verified using RSA-PSS variant
            try
            {
                PSSSigner signer = new PSSSigner(new RSAEngine(), new SHA256Digest(), 32);
                signer.init(false, rsaPk);
                signer.update(message, 0, message.length);
                b = signer.verifySignature(signature.getSignature());
        ...
        {
            // other RSA signatures (e.g., certificates stored in keystores) are verified using SHA256WITHRSA (non-PSS variant)
            Digest digest = new SHA256Digest();

            RSADigestSigner verifier = new RSADigestSigner(digest);

            verifier.init(false, rsaPk);

            // Update the signer with the data to be signed
            verifier.update(message, 0, message.length);

            b = verifier.verifySignature(signature.getSignature());
        }
